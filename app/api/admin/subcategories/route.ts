...
export async function GET(req: Request) {
  try {
    ...
    let query = admin.from('subcategories').select('*');
    if (categoryId) {
...
export async function POST(req: Request) {
  try {
    ...
    if (!category_id || !name || !slug) {
      return NextResponse.json({ ok: false, error: 'Category ID, name, and slug are required' }, { status: 400 });
    }
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ ok: false, error: 'Subcategory ID is required' }, { status: 400 });
    }
